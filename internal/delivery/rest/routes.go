...
func LoadRoutes(e *echo.Echo, h *handler, rdb *redis.Client) {
	...
	categoryGroup.POST("", h.CreateCategory, am.RequiredRoles(string(model.UserRoleAdmin)), middleware.Idempotency(rdb))
	...
	productGroup.POST("", h.CreateProduct, am.RequiredRoles(string(model.UserRoleAdmin)), middleware.Idempotency(rdb))
	...
	memberGroup.POST("", h.CreateMember, am.RequiredRoles(string(model.UserRoleAdmin)), middleware.Idempotency(rdb))
	...
	taxGroup.POST("", h.CreateTax, am.RequiredRoles(string(model.UserRoleAdmin)), middleware.Idempotency(rdb))
